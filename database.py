...
def migrate_old_sqlite_data(pg_cursor, pg_conn) -> None:
    ...
    if os.path.exists(OLD_DB_FILE) and os.path.getsize(OLD_DB_FILE) > 0:
        ...
        try:
            with sqlite3.connect(OLD_DB_FILE) as lite_conn:
...
def check_and_remove_expired_titles(telegram_id: int, external_cursor=None) -> bool:
    """
...
def get_or_create_user(telegram_id: int, username: Optional[str]) -> Any:
    ...
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
            try:
                # Run expiration check utilizing the same live atomic connection pipeline
                check_and_remove_expired_titles(telegram_id, external_cursor=cursor)
                ...
                if user_record and not isinstance(user_record, dict) and not IS_POSTGRES:
...
def update_stats(telegram_id: int, score_change: int, mode: str = 'win', external_cursor=None) -> Optional[Dict[str, Any]]:
    """
    ...
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
            try:
                ...
                leaderboard_dataset = [dict(r) if not isinstance(r, dict) else r for r in records]
            except Exception as error:
    ...
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
            try:
                ...
                records = cursor.fetchall()
                catalog_items = [dict(r) if not isinstance(r, dict) else r for r in records]
            except Exception as error:
...
def create_duel_link(player_id: int, opponent_id: int, initial_status: str = "pending") -> int:
    ...
    Verifies that neither player is already engaged in another duel.
    """
    created_id = -1
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
            try:
                ...
                cursor.execute(adapt_query("SELECT player_status FROM users WHERE telegram_id = ?;"), (player_id,))
                p1 = cursor.fetchone()
                cursor.execute(adapt_query("SELECT player_status FROM users WHERE telegram_id = ?;"), (opponent_id,))
                p2 = cursor.fetchone()

                if p1 and p1['player_status'] != 'idle' or p2 and p2['player_status'] != 'idle':
                    return -2  # Player Busy Error State
                ...
                # Lock statuses into confirmation loop phases
                cursor.execute(adapt_query("UPDATE users SET player_status = 'requested', current_duel_id = ? WHERE telegram_id = ?;"), (created_id, player_id))
                cursor.execute(adapt_query("UPDATE users SET player_status = 'challenged', current_duel_id = ? WHERE telegram_id = ?;"), (created_id, opponent_id))
...
def accept_duel(link_id: int) -> bool:
    """
    Transitions a duel request link into an active match status safely.
    """
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
            try:
                cursor.execute(adapt_query("SELECT player_id, opponent_id FROM duel_links WHERE id = ? AND status = 'pending';"), (link_id,))
                duel = cursor.fetchone()
                if not duel:
                    return False

                cursor.execute(adapt_query("UPDATE duel_links SET status = 'active' WHERE id = ?;"), (link_id,))
                cursor.execute(adapt_query("UPDATE users SET player_status = 'fighting' WHERE telegram_id IN (?, ?);"), (duel['player_id'], duel['opponent_id']))
                
                connection.commit()
                return True
            except Exception as error:
                connection.rollback()
                logger.error(f"Failed to process administrative duel acceptance logic: {error}")
                return False

def reject_or_cancel_duel(link_id: int, action_type: str = "rejected") -> bool:
    """
    Cancels or rejects a pending duel link request, resetting player statuses back to idle.
    """
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
            try:
                cursor.execute(adapt_query("SELECT player_id, opponent_id FROM duel_links WHERE id = ?;"), (link_id,))
                duel = cursor.fetchone()
                if not duel:
                    return False

                cursor.execute(adapt_query("UPDATE duel_links SET status = ? WHERE id = ?;"), (action_type, link_id))
                cursor.execute(adapt_query("UPDATE users SET player_status = 'idle', current_duel_id = NULL WHERE telegram_id IN (?, ?);"), (duel['player_id'], duel['opponent_id']))
                
                connection.commit()
                return True
            except Exception as error:
                connection.rollback()
                logger.error(f"Failed to reset duel structural linkage models: {error}")
                return False

def end_duel(link_id: int, winner_id: int, points_pool: int = 50) -> bool:
    """
    Finalizes an active match link record, updates user tier structures, and unlinks slots.
    """
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
            try:
                cursor.execute(adapt_query("SELECT player_id, opponent_id, status FROM duel_links WHERE id = ?;"), (link_id,))
                duel = cursor.fetchone()
                if not duel or duel['status'] != 'active':
                    return False

                p1, p2 = duel['player_id'], duel['opponent_id']
                loser_id = p2 if winner_id == p1 else p1

                # Log final operational metrics tracking details
                cursor.execute(adapt_query("UPDATE duel_links SET status = 'finished', winner_id = ? WHERE id = ?;"), (winner_id, link_id))
                
                # Apply score stat balances via integrated inline transaction routing calls
                update_stats(winner_id, points_pool, mode='win', external_cursor=cursor)
                update_stats(loser_id, -points_pool, mode='loss', external_cursor=cursor)

                # Reset structural player profiles statuses cleanly
                cursor.execute(adapt_query("UPDATE users SET player_status = 'idle', current_duel_id = NULL WHERE telegram_id IN (?, ?);"), (p1, p2))
                ...
                connection.rollback()
                logger.error(f"Failed finalizing structural duel outcome sequence matrix: {error}")
                return False
    ...
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
            try:
                ...
                record = cursor.fetchone()
                if record:
                    data_payload = dict(record) if not isinstance(record, dict) else record
            except Exception as error:
    ...
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
            try:
                ...
                record = cursor.fetchone()
                if record:
                    event_packet = dict(record) if not isinstance(record, dict) else record
            except Exception as error:
    ...
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
            try:
                ...
                records = cursor.fetchall()
                items_list = [dict(row) if not isinstance(row, dict) else row for row in records]
            except Exception as error:
    ...
    with get_db_connection() as connection:
        with connection.cursor() as cursor:
            try:
                ...
                records = cursor.fetchall()
                elite_dataset = [dict(row) if not isinstance(row, dict) else row for row in records]
            except Exception as error:
